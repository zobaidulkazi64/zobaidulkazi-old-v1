...
const SkillSection: React.FC<SkillSectionProps> = ({ skillsData, theme }) => {
  ...
  return (
    <article
      className={`h-max rounded-2xl p-12 pt-32 grid grid-cols-3 gap-10 relative z-10 max-lg:w-full max-lg:grid-cols-2 max-lg:p-8  max-lg:pt-32 ${
        theme === "dark"
    ...
    >
      ...
      {skillsData[0].skills.map((skill, index) => (
        <div
          key={index}
          className={`skill-item cursor-pointer flex flex-col gap-5 rounded-2xl p-8 border-solid border-[0.25rem]  text-center max-lg:items-center  mt-5   ${
